...
import { useAllFindings, useEngagements } from '../hooks/useEngagement'
...
export default function GlobalFindings() {
  ...
  const engMap = Object.fromEntries(engagements.map(e => [e.id, e.name]))
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-end gap-3">
        <div>
          ...
          <p className="text-[12px] font-mono text-fg-muted mt-0.5">
            All vulnerabilities across every engagement
          </p>
        </div>
        <div
          className="h-px flex-1 mb-1.5"
      ...
      {!isLoading && sorted.length === 0 && (
        <HoloCard className="hud-card text-center py-12" corners>
          <Bug size={24} className="mx-auto text-fg-dim mb-3" />
      ...
      <div className="space-y-1">
        {sorted.map(f => (
          <div key={f.id}>
            {engMap[f.engagement_id] && (
              <div className="flex items-center gap-2 mb-1 px-1">
                <span className="text-[10px] font-display tracking-widest text-fg-dim">ENGAGEMENT</span>
                <Link
                  to={`/engagements/${f.engagement_id}`}
                  className="text-[11px] font-mono text-fg-muted hover:neon-cyan transition-colors"
                >
                  {engMap[f.engagement_id]}
                </Link>
              </div>
